...
def generate_test_data(
    ...
    metric_control_numerator = np.abs(
        np.random.normal(loc=1, scale=std, size=control_num_units)
    )
    metric_control_denominator = np.abs(
        np.random.normal(loc=1, scale=std, size=control_num_units)
    )

    metric_treatment_numerator = np.abs(
        np.random.normal(
            loc=1 + treatment_effect, scale=std, size=treatment_num_units
        )
    )
    metric_treatment_denominator = np.abs(
        np.random.normal(loc=1, scale=std, size=treatment_num_units)
    )
...
#     assert_p_value_distribution_under_null(method="ztest", num_units=1000, num_sims=10000)
